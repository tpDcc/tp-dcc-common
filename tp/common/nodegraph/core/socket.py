...
from tp.common.nodegraph.core import consts, utils, exceptions, datatypes, commands
...
class Socket(object):
	...
	# 	node = self.get_node()
	#
	# 	if self.get_direction() == consts.SocketDirection.Input:
	# 		return node.get_input_data(self)
	# 	else:
	# 		return node.get_data(self)
	#
	# def set_value(self, value):
	# 	"""
	# 	Sets socket value.
	#
	# 	:param value: socket value.
	# 	"""
	#
	# 	node = self.get_node()
	# 	if self.data_type == datatypes.DataTypes.EXEC:
	# 		return
	# 	node.set_property(self.get_name(), value)
