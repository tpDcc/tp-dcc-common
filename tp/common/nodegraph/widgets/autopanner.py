    ...
    def tick(self, rect, pos):
        if self._is_allowed:
            if pos.x() < 0:
                ...
                self._amount = mathlib.clamp(abs(pos.x()) * 0.1, 0.0, 25.0)
            if pos.x() > rect.width():
                self._auto_pan_delta = QVector2D(self._amount, 0.0)
                self._been_outside = True
                self._amount = mathlib.clamp(abs(rect.width() - pos.x()) * 0.1, 0.0, 25.0)
            if pos.y() < 0:
                self._auto_pan_delta = QVector2D(0.0, -self._amount)
                self._been_outside = True
                self._amount = mathlib.clamp(abs(pos.y()) * 0.1, 0.0, 25.0)
