...
from Qt.QtGui import QColor, QPen, QBrush, QTransform, QLinearGradient
...
def draw_default_connector(connector_view, painter, option, widget):
    ...
    start_color = QColor(*connector_view.get_color())
    end_color = None

    pen_style = connector_view.get_style()
    pen_width = connector_view.get_thickness()
    if connector_view.ready_to_slice:
        start_color = QColor(155, 0, 0, 255)
        pen_style = consts.ConnectorStyles.DOTTED
        pen_width = 1.5
    elif connector_view.source and connector_view.source.is_exec():
        start_color = QColor(*connector_view.source.get_color())
        pen_width = 2.0
        ...
        if connector_view.source and connector_view.target:
            start_color = QColor(*connector_view.source.get_color())
            end_color = QColor(*connector_view.target.get_color())
    ...
    if connector_view.source and connector_view.target:
        ...
        painter.drawPolygon(transform.map(connector_view.get_arrow()))
...
def draw_realtime_connector(connector_view, painter, option, widget):
    ...
    color = QColor(*connector_view.get_color())
    pen_style = consts.ConnectorStyles.DASHED
    pen_width = connector_view.get_thickness() + 0.35
    ...
    end_circle_rect = QRectF(target_point.x() - half_end_circle, target_point.y() - half_end_circle, end_circle_size, end_circle_size)
    painter.setBrush(color)
...
def draw_slicer_connector(slicer_view, painter, option, widget):
    ...
    color = QColor(*consts.DEFAULT_CONNECTOR_SLICER_COLOR)
    ...
    text_color = QColor(*consts.DEFAULT_CONNECTOR_SLICER_COLOR)
